...
impl Stack { fn add_move(&mut self, mv: Move) {
        self.moves.push(mv);
    }
}
...
fn solve_part1(mut input: Stack) -> String {
    for mv in input.moves {
        ...
        input.crates[mv.to - 1].append(&mut vec_to_process[vec_to_process.len()-mv.amount..].into_iter().rev().map(|c| *c).collect());
...
fn solve_part2(mut input: Stack) -> String {
    for mv in input.moves {
        let vec_to_process = input.crates[mv.from - 1].clone();
        input.crates[mv.to - 1].append(&mut vec_to_process[vec_to_process.len()-mv.amount..].into_iter().map(|c| *c).collect());
        if mv.amount == input.crates[mv.from - 1].len() {
            input.crates[mv.from - 1].clear();
        } else {
        input.crates[mv.from - 1] = vec_to_process[..vec_to_process.len()-mv.amount].to_vec();
...
fn split_input(input: &str) -> Stack {
    ...
    let number_of_cols: usize = last_line.split("   ").last().unwrap().trim().parse::<usize>().unwrap();
    let mut crates = vec![];
